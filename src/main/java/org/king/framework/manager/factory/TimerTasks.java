package org.king.framework.manager.factory;
...
import org.king.common.utils.IpUtils;
...
import org.king.project.monitor.operlog.service.IOperLogService;
...
public class TimerTasks {
    ...
    public static TimerTask saveExceLog(final String ip, final int status, final ExceLog exceLog) {
        return new TimerTask() {
            @Override
            public void run() {
                Email email = Kings.getEmail();
                if (status >= HttpServletResponse.SC_INTERNAL_SERVER_ERROR && email.isEnabled()) {
                    MailUtil.send(email.getSend(), "King系统的异常告警", exceLog.getContent(), false);
                }
