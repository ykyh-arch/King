package org.king.framework.shiro.service;

import javax.servlet.http.HttpServletResponse;

import org.king.common.cons.Constants;
import org.king.common.cons.ShiroConstants;
import org.king.common.cons.UserConstants;
import org.king.common.enums.UserStatus;
...
import org.king.framework.manager.ThreadExecutors;
import org.king.framework.manager.factory.TimerTasks;
import org.king.framework.spring.ApplicationUtils;
import org.king.project.system.user.domain.User;
import org.king.project.system.user.service.IUserService;
...
public class LoginService {
    ...
    public User login(String username, String password) {
        ...
        if (!StringUtils.isEmpty(ApplicationUtils.getRequest().getAttribute(ShiroConstants.CURRENT_CAPTCHA))) {
            ...
            throw new KingException(HttpServletResponse.SC_BAD_REQUEST, "验证码错误");
        }
        // 用户名或密码为空 错误
        if (StringUtils.isEmpty(username) || StringUtils.isEmpty(password)) {
            ThreadExecutors.execute(TimerTasks.recordLogininfor(username, Constants.LOGIN_FAIL, "用户名或密码为空"));
            throw new KingException(HttpServletResponse.SC_BAD_REQUEST, "用户名或密码为空");
        }
        // 密码如果不在指定范围内 错误
        ...
        if (user.getDeleted()) {
            ...
            throw new KingException(HttpServletResponse.SC_BAD_REQUEST, "对不起，您的账号已被删除");
        }

        if (UserStatus.DISABLE.getCode().equals(user.getStatus())) {
            ThreadExecutors.execute(TimerTasks.recordLogininfor(username, Constants.LOGIN_FAIL, "用户已被禁用，请联系管理员", user.getRemark()));
            throw new KingException(HttpServletResponse.SC_BAD_REQUEST, "用户已被禁用，请联系管理员");
        }
